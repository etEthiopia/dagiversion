...
class CinemaSchedule
{
    ...
    {
        Database::getInstance()->query("SELECT * FROM cinemas,movies,movie_schedules WHERE cinemas.id = :id AND movie_schedules.cinema_id=cinemas.id AND movies.id=movie_schedules.movie_id");
        Database::getInstance()->bind(':id', $data['id']);

        $row = Database::getInstance()->resultset();
        $this->cinemaName = $data['id'];
        foreach ($row as $movieSchedule) {
